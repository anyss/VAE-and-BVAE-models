...
#Fasta File Reader for peptides
def fasta_read_pep(fastafile, seq_read_counts=False, drop_duplicates=False):
    o = open(fastafile)
    titles = []
    seqs = []
    for line in o:
...
def get_rawdata_pep(data_file):
    data = fasta_read_pep(data_file)
    df = pd.DataFrame(data, columns =['sequence'])
    train, validate = train_test_split(df, random_state=42)
    return train, validate

# gets data from data directory
def get_rawdata(type, countcutoff, n_copies, round=8, outputnum=2, seed=69, abs_path= None):
    '''
    ...
    :return: (train, validate) datasets should be ready to be loaded for use in each model
    '''
    #expanse
    #abs_path = "/expanse/lustre/scratch/jonahowns/temp_project/ML_for_aptamers/data/"

    #agave
    # abs_path = "/scratch/jprocyk/ML/ML_for_aptamers/data/"

    #local
    #abs_path = "/home/aneta/pytorch_intro_peptide_VAE_1/data/"
    ...
    if 'N' in type:  # normal unadjusted data
        ...
        tmpdata = list(zip(tmp["sequence"].tolist(), tmp["affinity"].tolist()))
        pruneddata = [(s, a) for (s, a) in tmpdata if len(s) == 40]
        tot = len(tmpdata)
        print("Removed ", (tot - len(pruneddata)) / tot * 100, '%', 'of data. Not Length 40')
        pseqs, paffs = zip(*pruneddata)
        lpseqs = [x[0:20] for x in pseqs]
        rpseqs = [x[20:40] for x in pseqs]
        bseqs = lpseqs + rpseqs
        if 'L' in type:
            data = pd.DataFrame({'sequence': lpseqs, 'affinity': paffs})
        if 'R' in type:
            data = pd.DataFrame({'sequence': rpseqs, 'affinity': paffs})
        if 'B20' in type:
            paffs = paffs+paffs  # double list, l and r have same affinity
            data = pd.DataFrame({'sequence': bseqs, 'affinity': paffs})
        elif 'B' in type:
            data = pd.DataFrame({'sequence': pseqs, 'affinity': paffs})
    elif 'M' in type: # Marco Adjusted counts, all seqs of length 20
        if 'I' in type:
            if 'L' in type:
                tmp = pd.read_csv(abs_path+"inf_multi_left_8.csv")
                data = tmp[['sequence'], ['inferred_count']].copy()
            if 'R' in type:
                tmp = pd.read_csv(abs_path+"inf_multi_right_8.csv")
                data = tmp[['sequence'], ['inferred_count']].copy()
            if 'B20' in type:
                tmp = pd.concat((pd.read_csv(abs_path+"inf_multi_left_8.csv"),
                                 pd.read_csv(abs_path+"inf_multi_right_8.csv"))
                                )
                data = tmp[['sequence'], ['inferred_count']].copy()
            elif 'B'in type:
                tmpl = pd.read_csv(abs_path+"inf_multi_left_8.csv")
                tmpr = pd.read_csv(abs_path+"inf_multi_right_8.csv")
                seqs = [x + tmpr['sequence'].tolist()[xid] for xid, x in enumerate(tmpl['sequence'].tolist())]
                affs = [(x + tmpr['inferred_count'].tolist()[xid]) / 2 for xid, x in
                        enumerate(tmpl['inferred_count'].tolist())]  # average of inferred counts
                data = pd.DataFrame({'sequence': seqs, 'affinity': affs})

        if 'C' in type:
            if 'L' in type:
                tmp = pd.read_csv(abs_path+"inf_multi_left_8.csv")
                data = tmp[['sequence'], ['affinity']].copy()
            if 'R' in type:
                tmp = pd.read_csv(abs_path+"inf_multi_right_8.csv")
                data = tmp[['sequence'], ['affinity']].copy()
            if 'B20' in type:
                ...
                tmpr = pd.read_csv(abs_path+"inf_multi_right_8.csv")
                seqs = [x+tmpr['sequence'].tolist()[xid] for xid, x in enumerate(tmpl['sequence'].tolist())]
                affs = [(x+tmpr['affinity'].tolist()[xid])/2 for xid, x in enumerate(tmpl['affinity'].tolist())] #average of inferred counts
                data = pd.DataFrame({'sequence': seqs, 'affinity': affs})
    elif 'H' in type: # marco adjusted w/ hamming distances to remove those freeloader strands
        tmp = pd.read_csv(abs_path)
        left = tmp[['seq_l', 'nn_l', 'count_l', 'inferred_count_l']].copy()
        right = tmp[['seq_r', 'nn_r', 'count_r', 'inferred_count_r']].copy()
        # remove duplicates
        lseqs = left.drop_duplicates(subset=None, keep='first', inplace=False)
        rseqs = right.drop_duplicates(subset=None, keep='first', inplace=False)
        # remove freeloader strands
        if 'T' in type:
            ln = lseqs[lseqs['nn_l'] == 1]
            rn = rseqs[rseqs['nn_r'] == 1]
        else:
            ln = lseqs
            rn = rseqs
        if 'I' in type: # inferred
            ls = ln[ln['inferred_count_l'] != ''] # Drop seqs w/o inferred counts
            rs = rn[rn['inferred_count_r'] != ''] # Drop seqs w/o inferred counts
            if 'L' in type:
                data = pd.DataFrame({'sequence': ls['seq_l'].tolist(), 'affinity': ls['inferred_count_l'].tolist()})
            if 'R' in type:
                data = pd.DataFrame({'sequence': rs['seq_r'].tolist(), 'affinity': rs['inferred_count_r'].tolist()})
            if 'B20' in type:
                data = pd.DataFrame({'sequence': ls['seq_l'].tolist() + rs['seq_r'].tolist(),
                                     'affinity': ls['inferred_count_l'].tolist() + rs['inferred_count_r'].tolist()})
            elif 'B' in type:
                # Don't drop duplicates until we've joined the seqs:
                seqlist = list(map(lambda i, j: i + j, zip(tmp['seq_l'].tolist(), tmp['seq_r'].tolist())))
                afflist = list(map(lambda i, j: (i + j) / 2,
                                   zip(tmp['inferred_count_l'].tolist(), tmp['inferred_count_r'].tolist())))  # average of the counts
                data = pd.DataFrame({'sequence': seqlist, 'affinity': afflist})
                # remove any duplicates
                data.drop_duplicates(subset=None, keep='first', inplace=True)
        elif 'C' in type:
            if 'L' in type:
                data = pd.DataFrame({'sequence': ln['seq_l'].tolist(), 'affinity': ln['count_l'].tolist()})
            if 'R' in type:
                data = pd.DataFrame({'sequence': rn['seq_r'].tolist(), 'affinity': rn['count_r'].tolist()})
            if 'B20' in type:
                data = pd.DataFrame({'sequence': ln['seq_l'].tolist() + rn['seq_r'].tolist(),
                                     'affinity': ln['count_l'].tolist() + rn['count_r'].tolist()})
            elif 'B' in type:
                # Don't drop duplicates until we've joined the seqs:
                seqlist = list(map(lambda i, j: i + j, zip(tmp['seq_l'].tolist(), tmp['seq_r'].tolist())))
                afflist = list(map(lambda i, j: (i + j)/2, zip(tmp['count_l'].tolist(), tmp['count_r'].tolist()))) # average of the counts
                data = pd.DataFrame({'sequence': seqlist, 'affinity': afflist})
                # remove any duplicates
                data.drop_duplicates(subset=None, keep='first', inplace=True)

    # split into train/test split
    # Don't use all bad binders just (n_cpoies * (size of binders in respective set))
    ...
    if 'G' in type:
        gen_seqs = pd.read_csv(abs_path+'fake_sequences.csv')
...
class NAReader(Dataset):
    ...
    def __init__(self, dataset, max_length=40, base_to_id=None, shuffle=True):
        ...
        self.n_bases = 5 #CHANGED!! Was 4 before
...
class AAReader(Dataset):
    """
    Train: For each sample creates randomly a positive or a negative pair
    Test: Creates fixed pairs for testing
    """

    def __init__(self, dataset, max_length = 45 , base_to_id=None, shuffle=True): 
